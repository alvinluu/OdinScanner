...
-(void) searchForText:(NSString *)searchText
{
	...
	{		
		transactArray = [CoreDataService searchObjectsForEntity:@"OdinTransaction" 
												 withPredicate:[NSPredicate predicateWithFormat:@"sync == true and (id_number beginswith[c] %@ or item contains[c] %@)",searchText,searchText]
...
{	
    [self searchForText:idSearch.text];
...
- (void)readDataForTable
{
    ...
    transactArray = [CoreDataService searchObjectsForEntity:@"OdinTransaction" 
											 withPredicate:[NSPredicate predicateWithFormat:@"sync == true"] 
												andSortKey:@"timeStamp" 
										  andSortAscending:NO 
												andContext:managedObjectContext];	
    #ifdef DEBUG 
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	//}
	selectedItem = [transactArray objectAtIndex:indexPath.row];
#ifdef DEBUG
	NSLog(@"Tran item %@ %@ %@",selectedItem.item, selectedItem.cc_approval, selectedItem.cc_tranid);
#endif
	if (selectedItem.cc_approval) {
...
{// pass selected item's indexPath to AddEditItemVC.
	if ([segue.identifier isEqualToString:@"voidCreditCard"])
	{
		...
		editItemVC.selectedItem = selectedItem;
	}
}
@end

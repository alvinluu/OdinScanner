...
#import "ReviewUploadsViewController.h"
#import "OdinTransaction.h"
#import "OdinEvent.h"

@interface ReviewUploadsViewController ()
...
@implementation ReviewUploadsViewController
...
-(void)connectionState:(int)state 
{
	switch (state) {
		...
		case CONN_CONNECTED:
            ...
            [[DTDevices sharedDevice] barcodeSetScanButtonMode:0 error:nil];
            //Turn on the beep 
...
-(void) searchForText:(NSString *)searchText
{
	...
	{		
		transactArray = [CoreDataHelper searchObjectsForEntity:@"OdinTransaction" 
												 withPredicate:[NSPredicate predicateWithFormat:@"sync == true and id_number beginswith[c] %@",searchText] 
													andSortKey:@"plu" 
											  andSortAscending:NO 

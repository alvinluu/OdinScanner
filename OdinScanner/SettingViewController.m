...
- (void)viewDidAppear:(BOOL)animated
{
	...
	devicDetail = [devicDetail stringByAppendingString:[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"]];
	...
	if ([dtdev serialNumber]) {
		...
		devicDetail = [devicDetail stringByAppendingString:@"Scanner is not active"];
	devicDetail = [devicDetail stringByAppendingString:@"\nDatabase: "];
...
-(NSString *)getFirmwareFileName
{
    ...
    NSString *path=[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"Firmware"];
#ifdef DEBUG
	NSLog(@"device name:%@ model:%@",dtdev.deviceName, dtdev.deviceModel);
	NSLog(@"firmware filename: %@ %@",path,name);
#endif
